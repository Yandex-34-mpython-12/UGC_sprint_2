...
async def _create_post(
    dto: PostCreateDto, post_svc: PostService = Depends(get_post_service)
) -> Post:
...
async def _delete_post(
    id: UUID, post_svc: PostService = Depends(get_post_service)
) -> None:
    post = await post_svc.get_by_id(post_id=id)
    if not post:
        raise HTTPException(
...
async def _create_like(
    dto: LikeCreateDto, post_svc: PostService = Depends(get_post_service)
) -> Like:
